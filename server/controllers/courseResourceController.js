...
export const getCourseResources = async (req, res) => {
  try {
    const resource = await CourseResource.findOne({ course: req.params.courseId }).populate('course');
    if (!resource) return res.status(404).json({ message: 'No resources found for this course' });
